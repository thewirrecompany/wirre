-- Fix assign_peer_reviews: update reference from finished_at -> coding_finished_at
-- (finished_at was renamed in 20260308030000_cleanup_unused_columns.sql)

DROP FUNCTION IF EXISTS public.assign_peer_reviews(uuid);
...
BEGIN
  ...
  -- For SAMPLE rounds: only include candidates who have finished coding (coding_finished_at IS NOT NULL)
  ...
  WITH shuffled_participants AS (
    ...
    WHERE assessment_id = target_assessment_id
      ...
      AND (
        (v_is_sample IS TRUE AND coding_finished_at IS NOT NULL)
        OR
